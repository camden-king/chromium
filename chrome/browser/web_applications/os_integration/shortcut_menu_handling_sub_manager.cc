...
void ShortcutMenuHandlingSubManager::Configure(const AppId& app_id,
    ...
    icon_manager_->ReadAllShortcutMenuIconsWithTimestamp(app_id, base::BindOnce(&ShortcutMenuHandlingSubManager::StoreShortcutMenuData, weak_ptr_factory_.GetWeakPtr(), std::ref(desired_state), title, url).Then(std::move(configure_done)));
}
...
void ShortcutMenuHandlingSubManager::StoreShortcutMenuData(proto::WebAppOsIntegrationState& desired_state, std::string title, std::string url, WebAppIconManager::ShortcutIconDataVector data) {
      for (auto & menu_item : data) {
            proto::ShortcutMenu* new_shortcut_menu_item = desired_state.add_shortcut_menu();
            ...
            for (const auto& [size, time] : menu_item[IconPurpose::ANY]) {
                  web_app::proto::ShortcutIconData* icon_data = new_shortcut_menu_item->add_icon_data_any();
            ...
            for (const auto& [size, time] : menu_item[IconPurpose::MASKABLE]) {
                  web_app::proto::ShortcutIconData* icon_data = new_shortcut_menu_item->add_icon_data_maskable();
                  icon_data->set_icon_size(size);
                  icon_data->set_timestamp(syncer::TimeToProtoTime(time));
            }

            for (const auto& [size, time] : menu_item[IconPurpose::MONOCHROME]) {
                  web_app::proto::ShortcutIconData* icon_data = new_shortcut_menu_item->add_icon_data_monochrome();
                  icon_data->set_icon_size(size);
                  icon_data->set_timestamp(syncer::TimeToProtoTime(time));
            }
      }
