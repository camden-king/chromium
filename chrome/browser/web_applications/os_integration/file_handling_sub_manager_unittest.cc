...
class FileHandlingSubManagerTest
 ...
 public:
  ...
  web_app::AppId InstallWebApp(apps::FileHandlers file_handlers) {
...
TEST_P(FileHandlingSubManagerTest, TestConfigInstall) {
  ...
  const AppId& app_id = InstallWebApp(file_handlers);
  ...
  if (AreOsIntegrationSubManagersEnabled()) {
    ...
    EXPECT_EQ(file_handling.approval(),
              proto::ApiApprovalState::REQUIRES_PROMPT);
...
TEST_P(FileHandlingSubManagerTest, TestConfigUpdateApproval) {
  apps::FileHandlers file_handlers;
  ...
  provider().scheduler().PersistFileHandlersUserChoice(app_id, false,
                                                       future.GetCallback());
  ...
  if (AreOsIntegrationSubManagersEnabled()) {
    ASSERT_TRUE(os_integration_state.has_file_handling());
    auto file_handling = os_integration_state.file_handling();
    EXPECT_EQ(file_handling.approval(), proto::ApiApprovalState::DISALLOWED);

    ASSERT_EQ(file_handling.file_handlers_size(), 1);
...
TEST_P(FileHandlingSubManagerTest, TestConfigUninstall) {
  apps::FileHandlers file_handlers;
